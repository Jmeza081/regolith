...
| P12 | Points of interest as Search filters: the chapter names the library repeats become chips; one narrows Search to the films carrying it and lists every occurrence | `ChapterFacet`, `UserChapterDao.observeFacets`/`filesWithChapter`/`occurrencesOf`, `SearchUiState.facets`/`poi`, `FilterChip` |
