...
| 2026-09-10 | The control that brings the wall back appears only at the collapsed anchor | At every other anchor the wall is on screen and there is nothing to restore, so a permanent button would be a control that usually does nothing. Collapsed is the one state where the handle alone is hard to find again, which is exactly where Gmail and Spotify put theirs, in the start corner of the pane that took over. |
| 2026-09-10 | The player's picture is divided into THIRDS, not halves: brightness, full screen, volume | Two halves left nowhere for a third gesture and put a dismiss under both of them. Thirds also match the app's own gesture map, which has always been drawn in three columns. A middle drag up enters full screen and down leaves it, committing past 12% of the height or on a flick, so a stray finger cannot flip the layout. |
