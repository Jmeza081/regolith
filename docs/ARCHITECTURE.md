...
| 2026-09-11 | Library roots inside a share: `share_roots` (schema v5), `Share.roots`, the Choose folders drill-down | A share is often `media/` with `Films/`, `Series/` and a `Backups/` nobody wants walked, and the only choice was the whole share or none. Rows in `share_roots` narrow a share to chosen folders; NO rows means the whole share, so every existing install reads exactly as before. The scan is untouched: `refreshFolder` on a narrowed share's root does not list SMB at all — it writes the chosen folders as direct children of the root row, each carrying its full path — so the walk, Browse and the Library see an ordinary tree and never know `Movies/Action` is two levels down. Un-picking a folder drops its row (and its files, through the FK) on the next visit. Picking a folder turns the share on; toggling the share whole clears the choice. The picker lists straight off the server (`SourceRepository.listFolders`) because nothing is scanned yet, one level per nav key so back climbs out a level and Done pops them all. |
...
| P2 | Library roots inside a share: the Choose folders drill-down, so a share can be narrowed to the folders worth walking | `share_roots` (schema v5), `Share.roots`, `SourceRepository.listFolders`, `RegolithKey.AddServer.Folders` |
