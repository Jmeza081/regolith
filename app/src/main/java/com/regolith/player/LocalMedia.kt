 ...
 *
 * Two things can answer yes: a finished download ("Keep on this device")
 * and the demo library. Everything that opens a file — the player, the
 ...
 * of the two it got, or that a demo library exists at all.
 *
...
class LocalMedia @Inject constructor(
    ...
    suspend fun file(fileId: Long): File? = demo.fileFor(fileId) ?: transfers.localFile(fileId)

    /** For ExoPlayer's loader and the frame decoder, which are already on their own threads. */
    fun fileBlocking(fileId: Long): File? = demo.fileFor(fileId) ?: transfers.localFileBlocking(fileId)
}
